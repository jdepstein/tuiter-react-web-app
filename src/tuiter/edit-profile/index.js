...
const EditProfileComponent = () => {
    const {loaded_profile, loading} = useSelector(state => state.profileData)
    const [profile, setProfile] = useState({})
    const dispatch = useDispatch();

    const callThunk = async () => {
        const {payload} = await dispatch(findProfileThunk());
        setProfile(payload)
    }
    useEffect(() => {
        callThunk()
                ...
                <div className="row position-relative mb-5">
                    ...
                    <div className="col-4">
                        <Link to="../edit-profile" className="wd-no-underline">
                            <button onClick={window.location.reload}  className="float-end btn rounded-pill border bg-large bg-white fw-bold wd-text-15px mt-2 me-2 text-white border-0 disabled"> Edit profile</button>
                        </Link>
                    </div>
                </div>
